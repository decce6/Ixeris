...
public class WindowRefreshCallbackStack {
    private static final Long2ReferenceMap<WindowRefreshCallbackStack> instance = Long2ReferenceMaps.synchronize(new Long2ReferenceOpenHashMap<>(1));
    ...
    public GLFWWindowRefreshCallbackI topCallback;

    private final long window;
    public volatile boolean suppressChecks;

    private WindowRefreshCallbackStack(long window) {
        this.window = window;
    }

    public static WindowRefreshCallbackStack get(long window) {
        return instance.computeIfAbsent(window, WindowRefreshCallbackStack::new);
    }
    
    public static void forEach(Consumer<WindowRefreshCallbackStack> consumer) {
