...
public class WindowSizeCallbackStack {
    private static final Long2ReferenceMap<WindowSizeCallbackStack> instance = Long2ReferenceMaps.synchronize(new Long2ReferenceOpenHashMap<>(1));
    ...
    public GLFWWindowSizeCallbackI topCallback;

    private final long window;
    public volatile boolean suppressChecks;

    private WindowSizeCallbackStack(long window) {
        this.window = window;
    }

    public static WindowSizeCallbackStack get(long window) {
        return instance.computeIfAbsent(window, WindowSizeCallbackStack::new);
    }
    
    public static void forEach(Consumer<WindowSizeCallbackStack> consumer) {
