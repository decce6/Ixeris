...
public class FramebufferSizeCallbackStack {
    private static final Long2ReferenceMap<FramebufferSizeCallbackStack> instance = Long2ReferenceMaps.synchronize(new Long2ReferenceOpenHashMap<>(1));
    ...
    public GLFWFramebufferSizeCallbackI topCallback;

    private final long window;
    public volatile boolean suppressChecks;

    private FramebufferSizeCallbackStack(long window) {
        this.window = window;
    }

    public static FramebufferSizeCallbackStack get(long window) {
        return instance.computeIfAbsent(window, FramebufferSizeCallbackStack::new);
    }
    
    public static void forEach(Consumer<FramebufferSizeCallbackStack> consumer) {
    ...
    public void onCallback(long window, int width, int height) {
        ...
        if (topCallback != null) {
            RenderThreadDispatcher.runLater(() -> {
                if (topCallback != null) {
                    topCallback.invoke(window, width, height);
                }
            });
        }
    }
